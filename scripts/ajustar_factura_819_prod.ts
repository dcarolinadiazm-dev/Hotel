...
async function ajustarFactura(numeroFactura: string, nitCliente: string) {
  console.log(`\n----------------------------------------------------`);
  console.log(`🏨 PROCESANDO FACTURA ${numeroFactura} (NIT: ${nitCliente})`);
  console.log(`----------------------------------------------------`);

  // 1. Buscar la Factura
  const factRow = await db('FACTURAS')
    .where('FACT_NUMERO', 'like', `%${numeroFactura}%`)
    .first();

  if (!factRow) {
    console.error(`❌ No se encontró la Factura ${numeroFactura} en la base de datos.`);
    return;
  }
  ...
  console.log(`✅ Factura encontrada: ID ${factId} | ${factPref}-${factNum} | Total: $${totalDoc}`);

  // 2. Buscar el Anticipo del cliente
  const antRow = await db('ANTICIPOS_CLIENTE')
    .where('TERC_NIT', nitCliente)
    .andWhere(function () {
      this.where('ANCL_ANULADO', '!=', 'S').orWhereNull('ANCL_ANULADO');
    })
    .orderBy('ANCL_ID', 'desc')
    .first();

  if (!antRow) {
    console.error(`❌ No se encontró el anticipo del cliente con NIT ${nitCliente}.`);
    return;
  }
  ...
  const anclBase = parseFloat(String(antRow.ANCL_BASE || totalDoc));
  ...
  console.log(`✅ Se eliminaron registros de pago en efectivo de FACTURAS_CONTADO_PAGO`);
  ...
  for (const rcd of rcDets) {
    ...
    console.log(`✅ Recibo de caja #${recaId} anulado`);
  }
...
async function run() {
  console.log('====================================================');
  console.log('🏨 AJUSTE DE FACTURAS Y CRUCE CON ANTICIPOS');
  console.log('====================================================');

  try {
    // Ajustar Factura 818
    await ajustarFactura('818', '13451004');
