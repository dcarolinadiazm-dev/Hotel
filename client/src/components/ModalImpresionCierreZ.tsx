      ...
      <div className="impresion-pos-dialog" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="impresion-pos-body">
          <div className="pos-ticket-paper" id="pos-ticket-cierre-z">
            ...
            {/* Abonos del Turno */}
            {data.detalleAbonosTurno && data.detalleAbonosTurno.length > 0 && (
              <>
                <div className="pos-divider-solid" />
                <div style={{ margin: '6px 0' }}>
                  <p style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '11px', textTransform: 'uppercase', marginBottom: '4px' }}>
                    ABONOS DEL TURNO ({data.detalleAbonosTurno.length})
                  </p>
                  <table className="pos-items-table">
                    <thead>
                      <tr>
                        <th style={{ textAlign: 'left', width: '25%' }}>RC / Hab</th>
                        <th style={{ textAlign: 'left', width: '45%' }}>Fecha / Huésped</th>
                        <th style={{ textAlign: 'right', width: '30%' }}>Monto</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.detalleAbonosTurno.map((a, i) => (
                        <tr key={i}>
                          <td style={{ textAlign: 'left', fontWeight: 'bold' }}>
                            #{a.reciboNumero}<br />
                            <span style={{ fontSize: '9px', fontWeight: 'normal' }}>Hab {a.habitacionNumero}</span>
                          </td>
                          <td style={{ textAlign: 'left' }}>
                            <span style={{ fontSize: '8.5px', color: '#333', fontWeight: 600 }}>{a.fecha || '—'}</span><br />
                            <span style={{ fontSize: '9px', color: '#555' }}>{(a.clienteNombre || '').slice(0, 16)}</span>
                          </td>
                          <td style={{ textAlign: 'right', fontWeight: 'bold' }}>
                    ...
                    <tfoot>
                      <tr style={{ borderTop: '1px dashed #000', fontWeight: 'bold' }}>
                        <td colSpan={2} style={{ textAlign: 'left' }}>TOTAL ABONOS</td>
                        <td style={{ textAlign: 'right' }}>
                          ${(data.totalAbonosTurno || 0).toLocaleString('es-CO')}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
            ...
            {/* Abonos Antiguos */}
            {data.detalleAbonosAntiguos && data.detalleAbonosAntiguos.length > 0 && (
              <>
                <div className="pos-divider-solid" />
                <div style={{ margin: '6px 0' }}>
                  <p style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '11px', textTransform: 'uppercase', marginBottom: '4px' }}>
                    ABONOS ANTIGUOS ({data.detalleAbonosAntiguos.length})
                  </p>
                  <table className="pos-items-table">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {data.detalleAbonosAntiguos.map((a, i) => (
                        <tr key={i}>
                          <td style={{ textAlign: 'left', fontWeight: 'bold' }}>
                            #{a.reciboNumero}<br />
                            <span style={{ fontSize: '9px', fontWeight: 'normal' }}>Hab {a.habitacionNumero}</span>
